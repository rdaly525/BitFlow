...
class NodePrinter1(Visitor):
    ...
    def generic_visit(self, node: DagNode):
        ...
        for child_node in node.children():
            ...
            #     print("&&&&&&&&&&&&&&&REDUCE")
            #
            #     #node = Add(node[0],node[1],name="trial")
            #
            #     print("add node 1")
            #     currSumNode = Add(node[0],node[1])
            #     returnNode = Round(currSumNode, Select(self.W, self.round_count),
            #                        name=node.name + "_round_W" + str(0))
            #     self.round_count += 1
            #     for i in range(2,node.size):
            #         print("add node"+str(i))
            #         currSumNode = Add(currSumNode,node[i])
            #         returnNode =  Round(currSumNode, Select(self.W, self.round_count),
            #                        name=node.name + "_round_W"+str(i))
            #
            #         self.round_count += 1
            #     print("&&&&&&&&&&&&&&&REDUCE")